...
class BNO055:
	...
	def begin(self, mode=None):
		...
		# self.writeBytes(BNO055.BNO055_PWR_MODE_ADDR, [BNO055.POWER_MODE_SUSPEND])
		time.sleep(0.01)

		# x = self.readBytes(BNO055.BNO055_PAGE_ID_ADDR)[0]
		# print("before", x, hex(x))
	...
	def get_2bytes(self, register):
		buf = self.readBytes(register, numBytes=2)
		value = ((buf[1]) << 8) | (buf[0])
		return value
	...
	def get_acc(self):
		# self.writeBytes(BNO055.BNO055_PAGE_ID_ADDR, [0])
		# time.sleep(0.1)
		acc_x = self.get_2bytes(BNO055.BNO055_ACCEL_DATA_X_LSB_ADDR)
		acc_y = self.get_2bytes(BNO055.BNO055_ACCEL_DATA_Y_LSB_ADDR)
		acc_z = self.get_2bytes(BNO055.BNO055_ACCEL_DATA_Z_LSB_ADDR)
		print(acc_x, acc_y, acc_z)

		buf = self.readBytes(BNO055.BNO055_ACCEL_DATA_X_LSB_ADDR, 6)
		xyz = struct.unpack('hhh', struct.pack('BBBBBB', buf[0], buf[1], buf[2], buf[3], buf[4], buf[5]))
		scalingFactor = 1.0
		vec = tuple([i/scalingFactor for i in xyz])
		print(vec)
...
if __name__ == '__main__':
	...
	# accel_rev, mag_rev, gyro_rev, sw_rev, bl_rev = bno.getRevInfo()
	# print(sw_rev)
	# aa = bno.readBytes(BNO055.BNO055_PAGE_ID_ADDR)
	# print(aa[0], hex(aa[0]))
